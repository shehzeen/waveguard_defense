...
def fetch_sample_music():
    ...
    # if not os.path.exists(wav_path):
    #     download(url, wav_path)
    fs, d = wavfile.read("test_adv.wav")
    print("Min, max", d.max(), d.min())
    d = d.astype('float32') / (2 ** 15)
    print("Min, max", d.max(), d.min(), d.shape, fs)
    # file is stereo - just choose one channel
    # d = d[:, 0]
    return fs, d
...
def lpc_analysis(X, order=8, window_step=128, window_size=2 * 128,
    ...
    for window in range(max(n_windows - 1, 1)):
        ...
        # print(lp_coefficients[window].shape, filter_coefs.shape)
    ...
    if residual_excitation is None:
        ...
        for window in range(n_windows):
            ...
            residual_excitation[index] += hanning * 0.01 * random_state.randn(
                window_size)
        # print("residual_excitation", residual_excitation)
    else:
...
def lpc_compress_decompress(X, samplerate = 16000, lpc_order = 10):
    ...
    print("LPC min max", noisy_lpc.min(), noisy_lpc.max())
    return np.array(noisy_lpc, dtype= 'float32')
...
def run_lpc_example():
    ...
    samplerate, X = fetch_sample_music()
    test_wav = lpc_compress_decompress(X)
    wavfile.write('lpc_test_function.wav', samplerate, soundsc(test_wav))
